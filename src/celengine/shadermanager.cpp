...
AddDirectionalLightContrib(unsigned int i, const ShaderProperties& props)
{
    ...
    {
        source += "float phaseAngle = acos(dot(" + LightProperty(i, "direction") + ", eyeDir));\n";
        // Rough match to the phase functions of non-Lambertian surfaces
        source += "float phaseFunction = exp(-phaseAngle);\n";
        source += "float lunarLambertWeight = 1 / (1 + 2 * (1 - lunarLambert) / (lunarLambert * phaseFunction));\n";
        source += "float normFactor = 1 + lunarLambertWeight;\n";
        source += AssignDiffuse(i, props) + " mix(NL, 2 * NL / (max(NV, 0.001) + NL), lunarLambertWeight) / normFactor;\n";
    }
    ...
    if (props.usesTangentSpaceLighting())
    {
        ...
        for (unsigned i = 0; i < props.nLights; i++)
        {
            ...
            if (util::is_set(props.lightModel, LightingModel::LunarLambertModel))
            {
                ...
                source += "float phaseAngle = acos(dot(" + LightDir_tan(i) + ", V));\n";
                // Rough match to the phase functions of non-Lambertian surfaces
                source += "float phaseFunction = exp(-phaseAngle);\n";
                source += "float lunarLambertWeight = 1 / (1 + 2 * (1 - lunarLambert) / (lunarLambert * phaseFunction));\n";
                source += "float normFactor = 1 + lunarLambertWeight;\n";
                source += "l = mix(NL, (2 * NL / (max(NV, 0.001) + NL)), lunarLambertWeight) / normFactor * clamp(" + LightDir_tan(i) + ".z * 8.0, 0.0, 1.0);\n";
            }
            else
            {
